...
class ServerOneClient extends Thread {
	...
	public void run() {
		try {
			log.writeLine("Client connected:" + socket.getInetAddress().toString());
		...
		try {
			log.writeLine("Client disconnected:" + socket.getInetAddress().toString());
			log.close();
		} catch (IOException | NullPointerException ex) {
			System.out.println("Failed to write log file");
		}
	...
	private void learningFromFile() {
		try {
			...
			try {
				ObjectInputStream serialIn = new ObjectInputStream(new FileInputStream("files/" + fileName + ".dmp"));
				out.writeObject("OK");
